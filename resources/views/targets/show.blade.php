    ...
    <div class="max-w-6xl mx-auto">
        <div class="flex justify-between items-center mb-8 border-b border-gray-700 pb-4">
            <div>
                ...
                <p class="text-gray-400">Target ID: #SX-00{{ $target->id }} | Status: <span class="text-yellow-500 uppercase">{{ $target->status }}</span></p>
            </div>
            <a href="{{ route('targets.index') }}" class="text-gray-400 hover:text-white border border-gray-600 px-4 py-2 rounded">← Back to Records</a>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
            <div class="bg-gray-800 p-6 rounded-lg border border-gray-700 shadow-xl">
                ...
                <div class="space-y-4">
                    <p><span class="text-gray-500 block text-xs">NAME</span> <span class="text-lg font-semibold">{{ $target->name }}</span></p>
                    <p><span class="text-gray-500 block text-xs">HANDLE</span> <span class="text-lg">{{ $target->username ?? 'Unknown' }}</span></p>
                    <p><span class="text-gray-500 block text-xs">EMAIL</span> <span class="text-md text-blue-300">{{ $target->email ?? 'Not Provided' }}</span></p>
                </div>
            </div>

            <div class="md:col-span-2 bg-gray-800 p-6 rounded-lg border border-blue-900 shadow-2xl">
                <h3 class="text-xl font-bold mb-4 text-green-400 border-b border-gray-700 pb-2">Automated Intelligence Gathering</h3>
